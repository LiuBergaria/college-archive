...
class DatabaseHelper {
  ...
  void _onCreate(Database db, int newVersion) async {
    await db.execute(
        'CREATE TABLE pessoa(id INTEGER PRIMARY KEY,nome TEXT, telefone TEXT)');
  }
...
// VALUES (\'${pessoa.nome}\', \'${pessoa.telefone}\')');
  ...
  Future<List> getPessoas() async {
    var dbClient = await db;
    var result =
        await dbClient.query("pessoa", columns: ["id", "nome", "telefone"]);
//var result = await dbClient.rawQuery('SELECT * FROM pessoa');
  ...
  Future<Pessoa> getPessoa(int id) async {
    ...
    List<Map> result = await dbClient.query("pessoa",
        columns: ["id", "nome", "telefone"], where: 'ide = ?', whereArgs: [id]);
    if (result.length > 0) {
